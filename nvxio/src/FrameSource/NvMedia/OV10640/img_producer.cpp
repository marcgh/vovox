    ...
    NvMediaIPPComponentOutput *output)
{
    ...
    LOG_DBG("Metadata %p: frameId %u, frame sequence #%lld, cameraId %u\n",
    ...
    NvMediaIPPComponentOutput *output)
{
    ...
    status = NvMediaEglStreamProducerPostMetaData(eglStrmProducerCtx->eglProducer[ippNum],          // producer
                                                  ...
                                                  4);                                // size
    if (status != NVMEDIA_STATUS_OK) {
        LOG_ERR("%s: NvMediaEglStreamProducerPostMetaData send Metadata size failed %d", __func__, ippNum);
    ...
    status = NvMediaEglStreamProducerPostMetaData(eglStrmProducerCtx->eglProducer[ippNum], // producer
                                                  ...
                                                  ltmDataSize);             // size
    if (status != NVMEDIA_STATUS_OK) {
        LOG_ERR("%s: NvMediaEglStreamProducerPostMetaData send Metadata failed %d", __func__, ippNum);
        *ctx->quit = NVMEDIA_TRUE;
        return status;
    }
