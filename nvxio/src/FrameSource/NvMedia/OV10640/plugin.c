    ...
    if (ctx->ispVersion == NVMEDIA_IPP_ISP_VERSION_4) {
        ...
        for (i=0; i<2; i++, ROIOffset_y += height/2) {
            ...
            for (j=0; j<2; j++, ROIOffset_x += width/2, index++) {
                ...
                numRemainder = width - (numPixels * numWindowsX * 2);
                ...
                streamSettings->lacSettings[0].v4.windows[index].size.width =
                            ...
                            numPixels >= 4  ? 4  :
                            numPixels >= 2  ? 2  : 0;
                streamSettings->lacSettings[0].v4.windows[index].startOffset.x = numRemainder / 2 + ROIOffset_x;
                numWindowsY = 32;
                numPixels = height / (numWindowsY * 2);
                numRemainder = height - (numPixels * numWindowsY * 2);
                ...
                streamSettings->lacSettings[0].v4.windows[index].size.height =
                            ...
                            numPixels >= 4  ? 4  :
                            numPixels >= 2  ? 2  : 0;
                streamSettings->lacSettings[0].v4.windows[index].startOffset.y = numRemainder / 2 + ROIOffset_y;
        ...
        streamSettings->lacSettings[0].v3.windows.size.width =
                    ...
                    numPixels >= 4  ? 4  :
                    numPixels >= 2  ? 2  : 0;
        streamSettings->lacSettings[0].v3.windows.startOffset.x = numRemainder / 2;
        ...
        streamSettings->lacSettings[0].v3.windows.size.height =
                    ...
                    numPixels >= 4  ? 4  :
                    numPixels >= 2  ? 2  : 0;
        streamSettings->lacSettings[0].v3.windows.startOffset.y = numRemainder / 2;
    ...
    int height)
{
    ...
    if (ctx->ispVersion == NVMEDIA_IPP_ISP_VERSION_4) {
        int i, j, index = 0;
        int ROIOffset_x = 0, ROIOffset_y = 0, numRemainder;
        ...
        for (i=0; i<2; i++, ROIOffset_y += height/2) {
            ...
            for (j=0; j<2; j++, ROIOffset_x += width/2, index++) {
                ...
                streamSettings->lacSettings[1].v4.windows[index].startOffset.x = numRemainder / 2 + ROIOffset_x;
                ...
                streamSettings->lacSettings[1].v4.windows[index].startOffset.y = numRemainder / 2 + ROIOffset_y;
        ...
        streamSettings->lacSettings[1].v3.windows.startOffset.x =
            (width - streamSettings->lacSettings[1].v3.windows.horizontalInterval * numWindowsX) / 2;
        ...
        streamSettings->lacSettings[1].v3.windows.startOffset.y =
            (height - streamSettings->lacSettings[1].v3.windows.verticalInterval * numWindowsY) / 2;
    ...
    int height)
{
    ...
    // If the height of the image is larger than
    // what can be covered by tightly placing the
    // bands vertically then calculate the VInterval
    // offset.
    if (numBands > 256) {
        offset = ((height - (windowHeight * 256)) / 255);
        numBands = 256;
    }
    ...
    if (ctx->ispVersion == NVMEDIA_IPP_ISP_VERSION_4) {
        ...
        streamSettings->flickerBandSettings.v4.windows.verticalInterval = offset;
        streamSettings->flickerBandSettings.v4.windows.startOffset.x = 0;
        streamSettings->flickerBandSettings.v4.windows.startOffset.y = 0;
        ...
        streamSettings->flickerBandSettings.v3.windows.verticalInterval = offset;
        streamSettings->flickerBandSettings.v3.windows.startOffset.x = 0;
        streamSettings->flickerBandSettings.v3.windows.startOffset.y = 0;
    } else {
        LOG_ERR("%s: isp version not supported\n", __func__, ctx->ispVersion);
    }
}
